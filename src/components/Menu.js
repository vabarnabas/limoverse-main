...
const Menu = (props) => {
    ...
    const incPage = () => {
        if (menuPage < (Math.ceil(menuIcons.length/((window.innerWidth > 480) ? 4 : 2))-1)) {
    ...
    return (
        <div className="relative flex items-center justify-center h-full flex-col sm:flex-row">
                ...
                {menuPage < (Math.ceil(menuIcons.length/((window.innerWidth > 480) ? 4 : 2))-1) &&
                    <HiArrowCircleRight onClick={incPage} className="text-white hover:text-gray-200 absolute right-5 text-4xl"/>
                }
            {menuIcons.slice(((window.innerWidth > 480) ? 0 + menuPage*4 : 0 + menuPage*2), ((window.innerWidth > 480) ? 4 + menuPage*4 : 2 + menuPage*2)).map((menu) => (
